...
echo"   <div class='menu'>
        ...
        <a title='Get last 30 days posts from limslb.com website - except أخبار اقتصادية' href='get_posts.php' class='button'>Retrieve posts</a>";
...
table.querySelectorAll('thead th').forEach((header, index) => {
  ...
  if (index === 2 || index === 4 || index === 6) {
    ...
    const values = Array.from(new Set(column.flatMap(td => td.textContent.trim().split(/\s*,\s*/))));
    uniqueValues[index] = values;

    // Create dropdown menu for each column
...
function filterTable() {
  ...
  tableRows.forEach(row => {
    const matches = filters.every((filter, index) => {
      if (index != 2 && index != 4 && index != 6) {
        return true; // Skip first two columns
      } else {
        const cellValues = row.cells[index * 2 + 1].textContent.trim().toLowerCase().split(',');
        return filter === '' || cellValues.map(value => value.trim().toLowerCase()).includes(filter);
      }
    });
